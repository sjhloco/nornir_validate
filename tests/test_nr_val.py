...
def load_inv_and_data():
    ...
    nr = InitNornir(inventory={"plugin": "SimpleInventory",
                         	                 "options": {"host_file": os.path.join(test_inventory, "hosts.yml"),
...
class TestNornirValidate:
    ...
    def test_template_task(self):
        ...
        desired_output = {}
        actual_output = {'show ip ospf neighbor': {'_mode': 'strict', '192.168.255.1': {'state': 'FULL'}, '2.2.2.2': {'state': 'FULL'}}}
        nr.run(task=template_task, tmpl_path=template_dir, input_vars=input_vars['hosts']["TEST_HOST"], desired_state=desired_output)
    ...
    def test_input_task(self):
        ...
        actual_output = nr.run(task=self.input_task_nr_task, input_file=os.path.join(test_data, "input_data.yml"))
        ...
        actual_output = nr.run(task=self.input_task_nr_task, input_file=os.path.join(test_data, "bad_input_data.yml"))
    ...
    def test_actual_state_engine(self):
        ...
        actual_output = actual_state_engine(nr.inventory.hosts['TEST_HOST'], {"show ip ospf neighbor": None})
    ...
    def test_actual_state_cmds(self):
        ...
        for cmd_output, desired_output in zip(desired_actual_cmd.cmd_output.items(), desired_actual_cmd.actual_state.items()):
            actual_output = actual_state_engine(nr.inventory.hosts['TEST_HOST'], {cmd_output[0]: cmd_output[1]})
            assert actual_output == {desired_output[0]: desired_output[1]}, err_msg.format(desired_output[0])
...
class TestComplianceReport:
    ...
    def test_report(self):
        ...
        desired_state = {"show ip ospf neighbor": {"_mode": "strict", "192.168.255.1": {"state": "FULL"}},
        ...
        desired_output = {
                    ...
                    "report": {
                        ...
                        "complies": False,
                        "skipped": []}}
        assert actual_output == desired_output, "❌ compliance_report: Combining report of comply true and false failed"

#6a. REPORT_FILE: Test saving a report to file, validate the contents
    def test_report_file(self):
        report = {"show ip ospf neighbor": {
                    "complies": True,
                    "present": {"192.168.255.1": {"complies": True, "nested": True}},
                    "missing": [],
                    "extra": []}}
        report_file('TEST_HOST', test_data, report, True, [])
        filename = os.path.join(test_data, 'TEST_HOST' + '_compliance_report_' + str(datetime.date.today()) + '.json')
        assert os.path.exists(filename) == True,  "❌ report_file: Creation of saved report failed"
        desired_output = {"complies": True,
                        "skipped": [],
                        "show ip ospf neighbor": {
                            "complies": True,
                            "present": {"192.168.255.1": {"complies": True, "nested": True}},
        ...
        assert report_from_file == desired_output, "❌ report_file: Saved report contents are incorrect"

        #6b. REPORT_FILE: Updating existing file and compliance state
        report = {"show etherchannel summary": {
                            "complies": False,
                            "present": {"Po3": {"complies": True, "nested": True}},
                            "missing": [],
                            "extra": []}}
        report_file('TEST_HOST', test_data, report, False, [])
        desired_output = {"complies": False,
                        "skipped": [],
                        "show ip ospf neighbor": {
                            "complies": True,
                            "present": {"192.168.255.1": {"complies": True, "nested": True}},
        ...
        with open(filename, 'r') as file_content:
            report_from_file = json.load(file_content)
        assert report_from_file == desired_output, "❌ report_file: Extending report and updating compliance failed"
        os.remove(filename)
