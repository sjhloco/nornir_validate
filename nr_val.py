...
def _create_parser() -> Dict[str, Any]:
    ...
    parser.add_argument(
        "-d", "--directory", default=report_directory, help="Directory where to save the compliance report"
    )
    parser.add_argument(
        "-f", "--filename", default=input_file, help="Name (with full path) of the input Yaml file of validation variables"
    )
...
def validate_task(task: Task, input_file: str =input_file, directory: str =report_directory) -> str:
    ...
    for each_cmd in task.host['desired_state']:
        cmd_output[each_cmd] = task.run(task=netmiko_send_command,
...
if __name__ == '__main__':
    ...
    result = nr.run(task=validate_task, input_file=args['filename'], directory=args['directory'])
    print_result(result)
